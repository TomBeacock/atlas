...
template<typename T>
constexpr QuaternionT<T>::QuaternionT()
  : x(0), y(0), z(0), w(1)
{
}
...
template<typename T>
constexpr T &QuaternionT<T>::operator[](Size i)
{
    Debug::assert(
...
template<typename T>
constexpr const T &QuaternionT<T>::operator[](Size i) const
{
    Debug::assert(
        i >= 0 && i < 4, "Cannot access value, i should be in range [0-3].");
    switch (i) {
...
constexpr QuaternionT<T> operator*(
    const QuaternionT<T> &q1, const QuaternionT<T> &q2)
{
    return QuaternionT<T>(q1.w * q2.x + q1.x * q2.w + q1.y * q2.z - q1.z * q2.y,
