...
#ifdef ATLAS_GRAPHICS_D3D12
  #define ATLAS_CLIP_SPACE_ZERO_ONE
  #define ATLAS_COORDINATE_SYSTEM_LEFT_HANDED
#endif

#ifdef ATLAS_COORDINATE_SYSTEM_LEFT_HANDED
  #define ATLAS_COORDINATE_SYSTEM_Z_SIGN
#else
  #define ATLAS_COORDINATE_SYSTEM_Z_SIGN -
#endif
...
constexpr MatrixT<4, 4, T> orthographic(T width, T height, T front, T back)
{
    ...
    result[3][3] = 1;
#ifdef ATLAS_CLIP_SPACE_ZERO_ONE
    result[2][2]
        = ATLAS_COORDINATE_SYSTEM_Z_SIGN static_cast<T>(1) / (back - front);
    result[3][2] = -front / (back - front);
#else
    result[2][2]
        = ATLAS_COORDINATE_SYSTEM_Z_SIGN static_cast<T>(2) / (back - front);
...
constexpr MatrixT<4, 4, T> perspective(T fov, T aspect, T front, T back)
{
    ...
    result[2][3] = ATLAS_COORDINATE_SYSTEM_Z_SIGN 1;
#ifdef ATLAS_CLIP_SPACE_ZERO_ONE
